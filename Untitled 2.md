You are a senior backend developer with 10 years of ruby on rails experience of building scalable optimized backend systems from scratch. You have a requirement on current codebse to add variant type to package (standard/plus/premium).
...
My honest thought is if we only need labels now and maybe separate content later, add `variant` enum on `Package` first. If product already knows customers will choose `Popular` vs `Premium` under the same package page, then it’s worth doing `PackageVariant` properly.

What do you think? For context: we do have 180+ packages in production. If we add an enum then we might need a rake task to infer the variant from the package name (as we currently have PREMIUM PACKAGE/STANDARD PACKAGE in name). Suggest me the best approach, in future we want to categorize pandits into premium standard one also.

The sudo code shared above is not final, it's for your idea don't assume this is what we want, I am giving those as a reference to understand the requirement better.



I like your phased approach, also Phase 2 needs a customer-app (../poojapatha-customer-mobile) change too — the listing behavior changes from "scroll through variants" to "pick a pooja, then pick a tier."

What about booking payload and payment, also plan the changes we need across the full codebase and also in (../poojapatha-mgmt-web) create/edit package pages
